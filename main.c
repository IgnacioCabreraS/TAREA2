...
Map * importar(Map * mapaGeneral, Map * mapaPorMarca, Map * mapaPorSector){
    ...
    //Se pide el nombre del archivo y se cargan tres files.
    do{
    ...
    //Se cargan los 3 mapas de productos.
...
//Funcion para cargar todos los datos del csv al mapa General
...
Map * cargarPorNombre(FILE * file, Map * mapa){
    ...
    while (fgets (lineaArchivo, 1024, file) != NULL) {
        // Se crea un tipo producto que contiene nombre, marca, sector, stock y precio
        producto * productos = (producto*) malloc (sizeof(producto));
        
        for(i = 0; i <= 4; i++){
            // char para obtener lo que salga en el csv antes de una coma
            const char * aux = get_csv_field(lineaArchivo, i);
            // cada "i" representa la string que va antes de una coma en una linea del csv
            if(i==0){
    ...
    while (fgets (lineaArchivo, 1024, file) != NULL) {
        ...
        for(i = 0; i <= 4; i++){
            ...
            if(i == 1){
                ...
                //Se busca si la marca existe ya registrada en el mapa.
    ...
    while (fgets (lineaArchivo, 1024, file) != NULL) {
        ...
        for(i = 0; i <= 4; i++){
            ...
            if(i == 2){
                ...
                //Se busca si el sector existe ya registrado en el mapa.
                const char * M;
...
//Esta función agrega el producto el cual se ingresa el nombre del producto, si aquel producto existe se aumenta el 
//stock, en caso contrario se pide todos lo datos del producto.
...
void agregarProducto(Map * mapaGeneral, Map * mapaMarcas, Map * mapaTipos){
    ...
    if(P != NULL){
        ...
        //Se inserta todo el producto en la mapa general
        insertMap(mapaGeneral, (void *)nuevoP->nombre, nuevoP);

        //Se añade el producto a los mapas de sector y marca
...
void buscarPorSector(Map * mapSector, Map * mapaGeneral){
    ...
    //Buscamos si existe el sector que se busca en el mapa de marcas.
    List * L;
    L = searchMap(mapSector,nombre);

    if(L){
        //Si existe el sector, se crea un tipo producto en la lista asociada al sector asociado.
        producto * P = firstList(L);
        while(P != NULL){
            
            //Se muestra el nombre del producto de la lista
            printf("%s ,", P->nombre);
            //Se busca el producto en el mapa general para poder sacar sus datos y mostrarlos.
...
void buscarPorMarca(Map * mapMarcas, Map * mapaGeneral){
    ...
    //Buscamos si existe la marca que se busca en el mapa de marcas.
    List * L;
    L = searchMap(mapMarcas,nombre);
    
    if(L){
        //Si existe la marca, se crea un tipo producto en la lista asociada a la marca asociada.
        producto * P = firstList(L);
        while(P != NULL){
...
//Esta funcion busca y muestra el nombre del producto que se encuentra en el mapa general, en caso de que no este el producto,
//se muestra por pantalla que el producto no existe.
...
//al carro elminando el stock del mapa general, si no muestra un mensaje por pantalla y acaba el programa
...
void agregarACarro(Map * mapaGeneral, Map * mapaCarritos, Map * mapaPorSector, Map * mapaPorMarca){
    ...
    if(C != NULL){//EXISTE EL CARRO
    ...
    }else{ //No existe el carro en mapa de carros.

        //Se crea nuevo carro de compras
        printf("Nuevo carro agregado.\n");
        carroCompras* nuevoCarroCompras = (carroCompras*) malloc(30*sizeof(carroCompras));

        //Se pide el nombre del producto, se busca en mapa general si existe.
        char * nuevoProducto = (char*) malloc(30*sizeof(char));
        printf("Ingrese el nombre del producto a insertar en el carrito: ");
        scanf(" %[^\n]s]", nuevoProducto);
        ...
        if(P != NULL){//Existe el producto en mapa general
            
            //Producto nuevo que guarda los datos del producto encontrado en mapa general
            producto * productoParaCarro = (producto*)malloc(sizeof(producto));
            productoParaCarro->nombre = P->nombre;
            productoParaCarro->marca = P->marca;
            productoParaCarro->sector = P->sector;
            ...
            if(P->stock <= 0){
                printf("Agotaste este producto.\n");
                // Eliminando productos de mapa marca (Lista que guarda productos asociados a marca)
                List * L;
                L = searchMap(mapaPorMarca,P->marca);
                producto * productoMapaMarcas = firstList(L);
                ...
                //Eliminando Productos de mapa sector (Lista que guarda productos asociados a sector)
            ...
            //Se guarda carro con lista de productos en mapa de carros.
